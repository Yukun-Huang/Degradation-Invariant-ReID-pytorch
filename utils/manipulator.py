...
__all__ = ['train_boundary', 'project_boundary', 'linear_interpolate',
           'degradation_adjust', 'batch_degradation_adjust',
           'linear_interpolate_torch', 'random_interpolate_torch']


def degradation_adjust(d, s, s_tar, boundary):
    '''
    :param d: shape=[N,C]
    :param s_tar: shape=[N,1] or [N]
    :param boundary: shape=[1,C]
    :return: shape=[N,C]
    '''
    if s is None:
        s = d.mm(boundary.transpose(1, 0))  # [N,C] x [C,1] = [N,1]
    if len(s_tar.shape) == 1:
        s_tar.unsqueeze(-1) # [N] -> [N,1]
    return d + boundary * (s_tar - s)  # [N,C] + [1,C]*([N,1]-[N,1]) = [N,C]


def batch_degradation_adjust(d, s_tar, boundary, steps):
    '''
    :param d: shape=[N,C]
    :param s_tar: shape=[N,STEP]
    :param boundary: shape=[1,C]
    :return: shape=[N*STEP,C]
    '''
    s = d.mm(boundary.transpose(1, 0))      # [N, 1]
    d = d.unsqueeze(1).repeat(1, steps, 1)  # [N, STEP, C]
    d = d + (s_tar - s).unsqueeze(-1) * boundary.unsqueeze(0)
    d = d.reshape((d.size(0) * steps, -1))
    return d


def random_interpolate_torch(latent_code, boundary, params, steps=10):
    assert len(boundary.shape) == 2 and boundary.shape[1] == latent_code.shape[-1]
    assert steps % 2 == 0
    n_bs = latent_code.size(0)
    mean_hr, std_hr, _, _ = params['hr']
    mean_lr, std_lr, _, _ = params['lr']

    unispace_lr = torch.normal(mean_lr, torch.tensor(std_lr).expand(n_bs, steps//2))
    unispace_hr = torch.normal(mean_hr, torch.tensor(std_hr).expand(n_bs, steps//2))

    unispace = torch.cat((unispace_lr, unispace_hr), dim=1).to(latent_code.device)

    ori_distance = latent_code.mm(boundary.transpose(1, 0))
    unispace = (unispace - ori_distance).unsqueeze(-1)
    boundary = boundary.unsqueeze(0)
    latent_code = latent_code.unsqueeze(1).repeat(1, steps, 1)
    return latent_code + unispace * boundary


def linear_interpolate_torch(latent_code, boundary, start_distance=-3.0, end_distance=3.0, steps=10):
    assert len(boundary.shape) == 2 and boundary.shape[1] == latent_code.shape[-1]

    linspace = torch.linspace(start_distance, end_distance, steps).to(latent_code.device)
    linspace = linspace.unsqueeze(0).expand(latent_code.size(0), -1)

    ori_distance = latent_code.mm(boundary.transpose(1, 0))
    linspace = (linspace - ori_distance).unsqueeze(-1)
    boundary = boundary.unsqueeze(0)
    latent_code = latent_code.unsqueeze(1).repeat(1, steps, 1)
