...
__all__ = ['re_ranking_ecn']


def re_ranking_ecn(q_g_dist, q_q_dist, g_g_dist, k=25, t=3, q=8, method='rankdist'):
    """ Expanded Cross Neighbourhood distance based Re-ranking (ECN)
    :param k, t, q: ECN parmaters (defaults k=25, t=3, q=8)
    :param method:  rankdist (default- based on rank list comparison) or origdist (based on original euclidean dist)
    :return:
            re-ranked distance matrix [shape: #test x #query]
    """
    nQuery = q_q_dist.shape[0]
    ntest = g_g_dist.shape[0]

    orig_dist = np.concatenate(
      [np.concatenate([q_q_dist, q_g_dist], axis=1),
       np.concatenate([q_g_dist.T, g_g_dist], axis=1)],
      axis=0)   # (nq+ng) x (nq+ng)
    # change the cosine similarity metric to euclidean similarity metric, [-1, 1] to [0, 4]
    orig_dist = 2. - 2 * orig_dist
    orig_dist = np.power(orig_dist, 2).astype(np.float32)
    orig_dist = np.transpose(1. * orig_dist/np.max(orig_dist, axis=0))
