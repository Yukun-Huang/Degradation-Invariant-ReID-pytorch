...
class CircleLoss(nn.Module):
    def __init__(self, m: float=0.25, gamma: float=4) -> None:
...
class MyCircleLoss(nn.Module):
    def __init__(self, m: float=0.25, gamma: float=4) -> None:
        super(MyCircleLoss, self).__init__()
        self.m = m
        self.gamma = gamma
        self.soft_plus = nn.Softplus()

    def forward(self, normed_feat: Tensor, label: Tensor) -> Tensor:

        sp, sn = convert_label_to_similarity(normed_feat, label)
...
if __name__ == "__main__":
    ...
    criterion = CircleLoss(m=0.25, gamma=8)
